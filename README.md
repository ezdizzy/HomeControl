...
- **Site Rules** — блокировка сайтов (домены через DNS → NXDOMAIN) и IP/подсетей (через nftables) для всей сети. Каждое правило можно ограничить окном времени (дни недели, часы, диапазон дат) или временно приостановить («разрешить на час») с автовозобновлением.
...
| Блокировка IP-правил | nftables set `blocked_rule_v4` (destination drop) |
